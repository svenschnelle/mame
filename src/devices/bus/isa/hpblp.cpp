...
void isa8_hpblp_device::device_add_mconfig(machine_config &config)
{
	...
	dio16.irq1_out_cb().set(FUNC(isa8_hpblp_device::dio_irq1_w));
	dio16.irq2_out_cb().set(FUNC(isa8_hpblp_device::dio_irq2_w));
	dio16.irq3_out_cb().set(FUNC(isa8_hpblp_device::dio_irq3_w));
	dio16.irq4_out_cb().set(FUNC(isa8_hpblp_device::dio_irq4_w));
	dio16.irq5_out_cb().set(FUNC(isa8_hpblp_device::dio_irq5_w));
	dio16.irq6_out_cb().set(FUNC(isa8_hpblp_device::dio_irq6_w));
	dio16.irq7_out_cb().set(FUNC(isa8_hpblp_device::dio_irq7_w));
...
offs_t isa8_hpblp_device::get_bus_address(offs_t offset, uint16_t mem_mask) const
{
	if (mem_mask == 0xff)
		offset++;
	return offset;
...
bool isa8_hpblp_device::forward_to_host(offs_t offset) const
{
...
uint8_t isa8_hpblp_device::status_val(offs_t offset) const
{
	if (offset >= 0x200000 && offset <= 0x2fffff)
		return 0x30;
